...
void Fellow::Action(ENEMYVECTOR *u_enemy, FELLOWVECTOR *u_fellow, Map *mp) {
  if (seal_) {  // 封印状態なので通常攻撃
    ...
    switch (sp_) {
      case 120:  // ホイミスライム
        Hoimi(u_enemy, u_fellow, mp);  //ここでホイミと攻撃を分岐させる必要は無い。特技使用率やフラフラ行動はHoimi関数内部に含まれてる。
        break;
      case 606:  // おばけキノコ
        if (rand()%20<3) {  // 2014.5.28 15% wiki出典
...
void Enemy::Action(ENEMYVECTOR *u_enemy, FELLOWVECTOR *u_fellow, Map *mp, Fellow *dead) {
  // まずは攻撃可能位置に敵がいるか確認
  bool attackable[8];
  for (int i = 0; i < 8; i++) {
    attackable[i] = true;
  ...
  if (allObject.size() !=0) {  // SS1 攻撃できる場合攻撃 => 完成
    int objectPlace = allObject[rand()%allObject.size()];
    ...
    // std::cout << "cannot atack !" << std::endl;
    allObject.clear();
    for (int i = 0; i < 8; i++) {
    ...
    if (allObject.size() == 0) {  // SS4 角抜けにいない場合ランダムに移動 => 完成
      ...
      if ((num=mp->Movable(place_).size()) != 0) {  // 移動スペースがある場合。無い場合は終了。
        int place = mp->Movable(place_)[rand()%num];
    ...
    while (it_f != u_fellow->end()) {
      if (it_f->GetPlace() == objectPlace) {
        ...
        if (it_f->GetIncidentHP() < 0) {  // 死んだ場合
          ...
          // std::cout << "survived!" << std::endl;
          if (it_f->GetSP() == 206
              && mp->VacantSpace(objectPlace)
              && rand()%4==0) {  // 対象が仲間スモグルで、分裂スペースがあり、かつ25%を引いた場合
            // とりあえずランダムに分裂で実装
            bool flag = true;
            int random = 0;
            while (flag) {
...
void Fellow::DokuNoIki(ENEMYVECTOR *u_enemy, Map *mp) {
  ...
  if (allObject.size() != 0) {  // 攻撃可能な位置があったらランダムに決定
    objectPlace = allObject[rand()%(allObject.size())];
    // vector iterator
    vector<Enemy>::iterator it = u_enemy->begin();
    while (it != u_enemy->end()) {
...
void Fellow::Hoimi(ENEMYVECTOR *u_enemy, FELLOWVECTOR *u_fellow, Map *mp) {
  ...
  for (int i = 0; i < 8; i++) {
    if (mp->GetMP(benri(i, place_)) == 2 || mp->GetMP(benri(i, place_)) == 3) {
      ...
      vector<Fellow>::iterator it2 = u_fellow->begin();
      while (it2 != u_fellow->end()) {
        if (it2->GetPlace() == benri(i, place_)
            && it2->GetIncidentHP() < it2->GetMHP()) {
          count++;
          break;
        }
        ++it2;
  ...
  for (int i = 0; i < count; i++) {
    if (rand()%10 < 3) { // ホイミ率は30%
      flag = false;
      vector<Enemy>::iterator it_e = u_enemy->begin();
      while (it_e != u_enemy->end()) {
        for (int j = 0; j < 8; j++) {
          if (it_e->GetPlace() == benri(j, place_)) it_e->Get25();
        }
        ++it_e;
      }
      vector<Fellow>::iterator it_f = u_fellow->begin();
  ...
  if (flag && rand()%3 == 0) {  // 現状では33%の確率で殴る事にしておく。
      ...
      if (it->GetPlace() == objectPlace) {
        ...
        for (int i = 0; i < howmany; i++) {
          ...
          if (it->GetIncidentHP() < 0) {  // 死んだ場合
            ...
            u_enemy->erase(it);
            // std::cout << "nubmer Of Enemy dead: " << u_enemy->size() << std::endl;
            exp_ += 22;
            totalEXP_ += 22;
            LVupCheck();
            break;
          } else {  // 生き残った場合（分裂判定）
            if (mp->VacantSpace(objectPlace)&&rand()%4==0){  // 分裂スペースがあり、かつ25%を引いた場合
              // 完全には実装されていない関数
...
void Unit::Damage(double at) {
  if (rand()%100<92) {
    int dm = (int)floor(at*1.3*pow(35, df_)/pow(36, df_)*(112+rand()%32)/128 + 0.5);
    // std::cout << "\t" << at<< "\t"  << df_<< "\t"  << dm << std::endl;
    if (dm != 0) {
      hp_ -= dm;
    } else {
      hp_ -= 1;
...
Fellow::Fellow(int place, int sp, int lv, int dp, int speed, int atWeakenTimes, int dfWeakenTimes, bool seal) {
  id_ = -1;
  seal_ = seal;
  place_ = place;
  sp_ = sp;
...
void Direction(const int thisPlace, const int objectPlace, Map* mp, ENEMYVECTOR *u_enemy) {
  ...
  // マップ情報を読み込み、変換してtmpに代入（十字の場合は下、斜めのときは右下を基準にする）
  int tmp[8];  // 周囲の情報
  ...
  switch (objectPlace - thisPlace) {
    ...
    case 21:  // 左上
      ...
      break;
    default:
      std::cout << "Problem occured at division direction." << std::endl;
      exit(1);
  }
  ...
  // std::cout << "object : " <<  objectPlace << std::endl;
  // std::cout << "after 逆変換: " <<  future << std::endl;
